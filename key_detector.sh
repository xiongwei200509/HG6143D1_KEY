...
# 检查脚本更新
new_version_check() {
ONLINE_VERSION=$(curl -Ls 'https://api.github.com/repos/ptcry233/hg6143d1_key/releases/latest' | grep '\"tag_name\":' | sed -E 's/.*\"([^\"]+)\".*/\1/')
# 比较版本号
if [[ "${ONLINE_VERSION}" != "${version}" ]]; then
  warn "脚本需要更新！"
  read -p "您想现在更新吗？(y/n):" choice
    case $choice in
        Y|y)wget --no-check-certificate -O /tmp/key_detector.sh https://gh.ddlc.top/https://github.com/ptcry233/hg6143d1_key/releases/latest/download/key_detector.sh
  # 检查下载是否成功
    if [ $? -eq 0 ]; then
        mv /tmp/key_detector.sh .
        ok 脚本更新成功，正在重启
        chmod +x ./key_detector.sh && bash ./key_detector.sh
    else
        err "下载出错！请检查${yellow}网络连接${none}是否正常或者${yellow}权限${none}是否到位!${none}" 
    fi ;;
         N|n);;
     esac
else
